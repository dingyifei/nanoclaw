...
function createSchema(database: Database.Database): void {
  ...
  // Add conditions column if it doesn't exist (migration for existing DBs)
  try {
    database.exec(
      `ALTER TABLE scheduled_tasks ADD COLUMN conditions TEXT DEFAULT NULL`,
    );
  } catch {
    /* column already exists */
  }
...
export function createTask(
  ...
  db.prepare(
    `
    INSERT INTO scheduled_tasks (id, group_folder, chat_jid, prompt, script, schedule_type, schedule_value, context_mode, conditions, next_run, status, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `,
  ).run(
    task.id,
